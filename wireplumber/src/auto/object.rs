...
use crate::{ffi,Core,ObjectFeatures};
...
pub trait ObjectExt: IsA<Object> + 'static {
    ...
    fn activate<P: FnOnce(Result<(), glib::Error>) + 'static>(&self, features: ObjectFeatures, cancellable: Option<&impl IsA<gio::Cancellable>>, callback: P) {
        ...
        unsafe extern "C" fn activate_trampoline<P: FnOnce(Result<(), glib::Error>) + 'static>(_source_object: *mut glib::gobject_ffi::GObject, res: *mut gio::ffi::GAsyncResult, user_data: glib::ffi::gpointer) {
            ...
            ffi::wp_object_activate_finish(_source_object as *mut _, res, &mut error);
    ...
    fn activate_closure(&self, features: ObjectFeatures, cancellable: Option<&impl IsA<gio::Cancellable>>, closure: glib::Closure) {
        unsafe {
            ffi::wp_object_activate_closure(self.as_ref().to_glib_none().0, features, cancellable.map(|p| p.as_ref()).to_glib_none().0, closure.into_glib_ptr());
        }
    }
    ...
    fn test_active_features(&self, features: ObjectFeatures) -> bool {
        unsafe {
            from_glib(ffi::wp_object_test_active_features(self.as_ref().to_glib_none().0, features))
        }
    }
    ...
    fn test_supported_features(&self, features: ObjectFeatures) -> bool {
        unsafe {
            from_glib(ffi::wp_object_test_supported_features(self.as_ref().to_glib_none().0, features))
        }
    }
    ...
    fn connect_active_features_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, c"notify::active-features".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(notify_active_features_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
    ...
    fn connect_supported_features_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, c"notify::supported-features".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(notify_supported_features_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
