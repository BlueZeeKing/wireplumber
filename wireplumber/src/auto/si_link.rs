...
use crate::{ffi,Object,Properties,SessionItem,SiLinkable};
...
pub trait SiLinkExt: IsA<SiLink> + 'static {
    ...
    fn connect_link_properties_changed<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, c"link-properties-changed".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(link_properties_changed_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
