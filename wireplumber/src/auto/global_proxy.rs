...
use crate::{ffi,Object,Properties,Proxy};
...
pub trait GlobalProxyExt: IsA<GlobalProxy> + 'static {
    ...
    fn connect_permissions_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, c"notify::permissions".as_ptr() as *const _,
                Some(std::mem::transmute::<*const (), unsafe extern "C" fn()>(notify_permissions_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
